...
Every title slide ships an abstract decorative SVG positioned right and faded to
~50% opacity. The reference patterns below are taken from the source course
(workshops have no chapters/lessons — read these as **example motifs by
topic**, not a required taxonomy):

| Reference example | Motif                                             |
|-------------------|---------------------------------------------------|
...
## Diagram palettes (workshop-specific)
...
workshop's `spec/style/` override — not here, since they're specific to one
workshop.
