...
The body that goes through `scripts/validate.py` and into the aggregated
`posts/<template>-post.md` file contains **only the post text** — exactly what
would be pasted into LinkedIn. No frontmatter, no metadata, no commentary.

```
{hook line — line 1}

{body, in the shape of the chosen template}
...
The QA / source-grounding metadata (citations, pre-post checks) does NOT live
in this body. It goes into the post entry's collapsible `<details>Source
notes</details>` block, written by `scripts/append_post.py` from the
`meta.notes` field.
