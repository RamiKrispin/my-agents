...
| --- | --- | --- | --- |
| **course-builder** | 0.2.1 | `course-builder` skill (orchestrator) + `course-content-creator` and `course-qa` subagents + a theme-aware `course-slide-deck` skill, driven by `/course`. Builds a **course** (chapters/lessons with scripts + per-lesson slides) stage-by-stage from a `spec/` source of truth, with a continuity ledger, named style profiles (default `design-principles`), step-by-step or all-at-once builds, and an independent QA pass. See [its README](plugins/course-builder/README.md). | `/plugin install course-builder@my-agents` |
| **workshop-builder** | 0.1.1 | `workshop-builder` skill (orchestrator) + `workshop-content-creator` and `workshop-qa` subagents + a theme-aware `workshop-slide-deck` skill, driven by `/workshop`. Sibling of course-builder focused on **workshops**: agenda topics + **single combined slide deck by default** (per-topic split on request) under `slides/`, **no scripts**, and topic READMEs **only when asked**. Same `spec/` workflow, continuity ledger, named profiles, and independent QA. See [its README](plugins/workshop-builder/README.md). | `/plugin install workshop-builder@my-agents` |
| **newsletter** | 0.2.0 | `newsletter-builder` skill + `/newsletter` command. Drafts a weekly data science newsletter — the fixed *Open Source of the Week* → *New Learning Resources* → *Book of the Week* sections — from a set of links **or prepared section files** (produced by the `social-content` plugin), in the author's voice. Researches each link (GitHub metadata, JS-rendering reader for fetch-hostile pages like O'Reilly/YouTube, ISBN→book lookup), runs a QA pass, and saves a Substack-ready draft. Local `.md` paths in any slot skip research + draft and are inlined verbatim. See [its README](plugins/newsletter/README.md). | `/plugin install newsletter@my-agents` |
| **social-content** | 0.3.0 | `social-content` skill + `/social-post` command. Drafts a **LinkedIn post**, a **Reels / TikTok script**, **or a newsletter section** in the author's voice, sourced from a course-builder run, a workshop-builder run, a newsletter draft, or a free-form topic. Templates are user-curated example posts under `profiles/default/examples/<template>/` (list, lesson, contrarian, comparison, learning-resource, opinion) — the skill learns each template's hook and rhythm from real posts you drop in. **LinkedIn / Reels drafts aggregate into `posts/<template>-post.md` files** in the working directory, newest at the top, with QA / source-grounding notes embedded in a collapsible `<details>` block per entry. **Newsletter sections** (auto-classified open-source / learning / book from the source, or set explicitly via `newsletter-type:`) write to `posts/<slug>-newsletter.md` and can be injected directly into a newsletter pre-draft (`scripts/inject_section.py --new` or `--append`) — the section format spec is shared with the `newsletter` plugin via `imports:`. Override the default file with a `group: <filename.md>` arg. Optional infographic spec (designer-facing brief) or **drawio diagram** sidecar in `posts/assets/<slug>.<ext>`. Four offline zero-dependency helpers: `validate.py`, `validate_newsletter_section.py`, `build_drawio.py`, `append_post.py`, `inject_section.py`. See [its README](plugins/social-content/README.md). | `/plugin install social-content@my-agents` |
...
| **qr-generator** | 0.1.0 | `qr-generator` skill. Converts a URL (or any short string) into a scannable QR code PNG, **locally** — the input never leaves the machine. Backends in priority order: macOS CoreImage via Swift (zero deps, ships with the OS), Python `qrcode` / `segno` if installed, or `qrencode` CLI. Defaults to ~1500×1500 px at error-correction level **H** (~30% damage tolerance — survives feed compression and partial occlusion). Use anywhere a slide, infographic, flyer, or printed asset needs a scan target. See [its README](plugins/qr-generator/README.md). | `/plugin install qr-generator@my-agents` |
...
| `imports:` in `plugin.yaml`     | listed paths copied from another plugin's source into this plugin's built tree | *(skipped — only Claude Code plugins use shared assets)* |

Prompt bodies are identical across tools (both use `$ARGUMENTS`), so they pass
through unchanged.

The `imports:` field lets two plugins share a single source of truth for a
file or directory without duplicating it. Each entry has the form
`<src-relative-to-source/plugins> > <dst-relative-to-built-plugin>`. Used
today by `social-content` to import the newsletter section format spec and
assembly template from `newsletter`.
